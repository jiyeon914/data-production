...
for num in range(num_train):
    ...
    for q in range(Nt):
        ...
        if q == 0: Jkp = Jk[:] # 이 부분 id 겹치는지 확인 필요
        
        wk = ((1 - 0.5*nu*k_sq*dt)*wk - dt*(1.5*Jk - 0.5*Jkp))/(1 + 0.5*nu*k_sq*dt); wk[0,0] = 0
        Jkp[] = Jk[]

        if q % K == 0:
            wn = irfftn(wk, axes = (0,1), norm = 'forward'); wn = np.expand_dims(cp.asnumpy(wn), axis = (0,1))
            w = np.concatenate((w,wn), axis = 0)
    
    CreateData = os.path.join(Train_Dir, f"/Train{num+1} 2D HIT n={N} T={T} dt={dt:.4f} K={K} data.h5")
